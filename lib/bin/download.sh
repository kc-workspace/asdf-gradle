...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    ...
    url="https://services.gradle.org/distributions/gradle-{version}-$(download_type).zip"
  ...
  if [[ "$mode" == "git" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://services.gradle.org/distributions/gradle-{version}-$(download_type).zip.sha256"
